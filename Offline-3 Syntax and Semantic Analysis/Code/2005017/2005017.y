...
parameter_list  : parameter_list COMMA type_specifier ID{
    ...
    | error{
      ...
      errorMessage("Syntax error at parameter list of function definition",@1.first_line);
      $$->setErrorFlag();
    ...
    | expression_statement{
      ...
      logRule(rule);
      $$=parsing(@$,rule)->addSubordinate(parsing(@1,PRINTLN_R))->addSubordinate(parsing(@2,LPAREN_R))->addSubordinate(parsing(@3,"ID : "+$3->getName()));
      ...
      variable : ID 	{
        ...
        $$=parsing(@$,"variable : ID 	 ",$1->getVarType())->addSubordinate(parsing(@1,"ID : "+$1->getName()));
	 ...
	 | ID LTHIRD expression RTHIRD {
      ...
      $$=parsing(@$,rule,$1->getVarType())->addSubordinate(parsing(@1,"ID : "+$1->getName()))->addSubordinate(parsing(@2,LTHIRD_R))->addSubordinate($3)->addSubordinate(parsing(@4,RTHIRD_R));
...
factor	: variable {
logRule("factor	: variable ");$$=parsing(@$,"factor	: variable ",$1->getDataType(),$1->getValue())->addSubordinate($1);
}
	...
	| CONST_INT {logRule("factor : CONST_INT");$$=parsing(@$,"factor : CONST_INT ","INT",$1->getName())->addSubordinate(parsing(@$,"CONST_INT : "+$1->getName()));
...
logRule("factor : CONST_FLOAT");$$=parsing(@$,"factor : CONST_FLOAT ","FLOAT",$1->getName())->addSubordinate(parsing(@$,"CONST_FLOAT : "+$1->getName()));
  ...
  $$=parsing(@$,rule,$1->getDataType())->addSubordinate($1)->addSubordinate(parsing(@2,INC_R));

  } 
	| variable DECOP{
string rule="factor : variable DECOP";logRule(rule);
  $$=parsing(@$,rule,$1->getDataType())->addSubordinate($1)->addSubordinate(parsing(@2,DEC_R));
  ...
  | ID LPAREN argument_list RPAREN{
    ...
    $$=parsing(@$,rule,$1->getVarType())->addSubordinate(parsing(@1,"ID : "+$1->getName(),$1->getReturnType()))->addSubordinate(parsing(@2,LPAREN_R));
